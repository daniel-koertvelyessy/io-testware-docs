<nav class="hidden lg:block fixed w-56 right-0 bg-white border-l-2 py-1 z-10  border-main-800 overflow-auto"
     x-bind:class="{'lg:top-0':shrinkNav} "
     x-transition:enter.duration.500ms
     x-transition:leave.duration.400ms
>
    <span class="xl:text-lg ml-3 font-semibold text-slate-400 mb-0 xl:mb-3">Auf dieser Seite</span>
    @foreach($items as $item)
        <x-nav-doc-item level="{{ $item['level']??'0' }}"
                        link="{{ $item['link'] }}"
        >{{ $item['header'] }}</x-nav-doc-item>
    @endforeach
</nav>
